			 ...
			 const struct hrtimer *exclude)
{
	...
	if (active_mask & HRTIMER_ACTIVE_HARD) {
		...
		/*
		 * Skip initializing cpu_base->next_timer to NULL as we skip
		 * updating next_timer in below loop if the timer is excluded.
		 */
		if (!exclude)
			cpu_base->next_timer = next_timer;
		expires_next = __hrtimer_next_event_base(cpu_base, active,
							 expires_next, exclude);
	}
...
hrtimer_update_softirq_timer(struct hrtimer_cpu_base *cpu_base, bool reprogram)
{
	 ...
	 */
	expires = __hrtimer_get_next_event(cpu_base, HRTIMER_ACTIVE_SOFT, NULL);
